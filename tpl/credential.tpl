...
<form action="{$formCredential|escape:htmlall:'UTF-8'}" method="post">
	<input type="hidden" name="layout" value="credential" />
	<fieldset>
		...
		{foreach from=$credentialInputVar item=input}
			...
			<div class="margin-form">
				<input type="{$input.type|escape:htmlall:'UTF-8'}" name="{$input.name|escape:htmlall:'UTF-8'}" id="{$input.name|escape:htmlall:'UTF-8'}" value="{$input.value|escape:htmlall:'UTF-8'}" /> {if $input.required}<span style="color:red">{l s='*' mod='gointerpay'}</span>{/if} {$input.desc|escape:htmlall:'UTF-8'}
			</div>
		{/foreach}
